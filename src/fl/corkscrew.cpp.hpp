...
vec2f calculateLedPositionExtended(fl::u16 ledIndex, fl::u16 numLeds,
                                   float totalTurns, const Gap &gapParams,
                                   fl::u16 width, fl::u16 height) {
    FL_UNUSED(height);
    FL_UNUSED(totalTurns);

    // Check if gap feature is active AND will actually be triggered
    bool gapActive = (gapParams.num_leds > 0 && gapParams.gap > 0.0f &&
                      numLeds > static_cast<fl::u16>(gapParams.num_leds));

    if (!gapActive) {
        // Original behavior when no gap or gap never triggers
        const float ledProgress =
            static_cast<float>(ledIndex) / static_cast<float>(numLeds - 1);
        const fl::u16 row = ledIndex / width;
        const fl::u16 remainder = ledIndex % width;
        const float alpha =
            static_cast<float>(remainder) / static_cast<float>(width);
...
void calculateDimensions(float totalTurns, fl::u16 numLeds,
                         const Gap &gapParams, fl::u16 *width,
                         fl::u16 *height) {
    FL_UNUSED(gapParams);
    ...
    // If the grid would have more pixels than LEDs, adjust height to better
    // match
    if (calc_width * height_from_turns > numLeds) {
        // Calculate height that better matches LED count
        calc_height = static_cast<fl::u16>(fl::ceil(
            static_cast<float>(numLeds) / static_cast<float>(calc_width)));
    } else {
...
Corkscrew::Corkscrew(float totalTurns, fl::u16 numLeds, bool invert,
                     const Gap &gapParams)
    : mTotalTurns(totalTurns), mNumLeds(numLeds), mGapParams(gapParams),
      mInvert(invert) {
    fl::calculateDimensions(mTotalTurns, mNumLeds, mGapParams, &mWidth,
                            &mHeight);
    mOwnsPixels = false;
}

// Constructor with external pixel buffer
Corkscrew::Corkscrew(float totalTurns, fl::span<CRGB> dstPixels, bool invert,
                     const Gap &gapParams)
...
vec2f Corkscrew::at_no_wrap(fl::u16 i) const {
    ...
    //     position = calculateLedPositionExtended(invertedIndex, mNumLeds,
    //     mTotalTurns,
    //                                    mGapParams, mState.width,
    //                                    mState.height);
    // }
...
Tile2x2_u8_wrap Corkscrew::calculateTileAtWrap(float i) const {
    ...
    for (fl::u8 x = 0; x < 2; x++) {
        for (fl::u8 y = 0; y < 2; y++) {
            // For each pixel in the tile, map it to the cylinder so that each
            // subcomponent is mapped to the correct position on the cylinder.
...
CRGB *Corkscrew::rawData() {
    ...
    if (!mPixelStorage.empty()) {
        ...
        } else if (mPixelStorage.template is<
                       fl::vector<CRGB, fl::allocator_psram<CRGB>>>()) {
            return mPixelStorage
                .template get<fl::vector<CRGB, fl::allocator_psram<CRGB>>>()
                .data();
        }
    }
    ...
    if (!mPixelStorage.empty()) {
        if (mPixelStorage.template is<fl::span<CRGB>>()) {
            return mPixelStorage.template get<fl::span<CRGB>>();
                       ...
                       fl::vector<CRGB, fl::allocator_psram<CRGB>>>()) {
            auto &vec = mPixelStorage.template get<
                fl::vector<CRGB, fl::allocator_psram<CRGB>>>();
            return vec;
        }
    }
...
void Corkscrew::readFrom(const fl::Grid<CRGB> &source_grid,
    ...
    for (fl::size led_idx = 0; led_idx < mNumLeds; ++led_idx) {
        ...
        coord.x = FL_MAX(
            0, FL_MIN(coord.x, static_cast<fl::i16>(source_grid.width()) - 1));
        coord.y = FL_MAX(
            0, FL_MIN(coord.y, static_cast<fl::i16>(source_grid.height()) - 1));
...
void Corkscrew::clear() {
    ...
    if (mInputSurface) {
        mInputSurface->clear();
        // Don't reset the shared_ptr - the test expects the surface to remain
        // valid The surface is cleared but the memory is not freed
    }

    // Clear pixel storage if we own it (vector variant)
    if (!mPixelStorage.empty()) {
        if (mPixelStorage
                .template is<fl::vector<CRGB, fl::allocator_psram<CRGB>>>()) {
            auto &vec = mPixelStorage.template get<
                fl::vector<CRGB, fl::allocator_psram<CRGB>>>();
            vec.clear();
            // Note: fl::vector doesn't have shrink_to_fit(), but clear() frees
            // the memory
        }
    ...
    // Clear tile cache
    mTileCache.clear();
    // Note: fl::vector doesn't have shrink_to_fit(), but clear() frees the
    // memory
    mCacheInitialized = false;
}
...
void Corkscrew::draw(bool use_multi_sampling) {
    // The draw method should map from the rectangular surface to the LED pixel
    // data This is the reverse of readFrom - we read from our surface and
    // populate LED data
    ...
    if (use_multi_sampling) {
            ...
            for (fl::u8 x = 0; x < 2; x++) {
                for (fl::u8 y = 0; y < 2; y++) {
                    ...
                    if (pos.x < source_surface->width() &&
                        ...
                        r_accum +=
                            static_cast<fl::u32>(sample_color.r) * weight;
                        g_accum +=
                            static_cast<fl::u32>(sample_color.g) * weight;
                        b_accum +=
                            static_cast<fl::u32>(sample_color.b) * weight;
                        total_weight += weight;
                    }
                }
            }
        ...
        for (fl::size led_idx = 0; led_idx < mNumLeds; ++led_idx) {
            ...
            coord.x = FL_MAX(
                0, FL_MIN(coord.x,
                          static_cast<fl::i16>(source_surface->width()) - 1));
            coord.y = FL_MAX(
                0, FL_MIN(coord.y,
                          static_cast<fl::i16>(source_surface->height()) - 1));
...
void Corkscrew::readFromMulti(const fl::Grid<CRGB> &source_grid) const {
    ...
    auto target_surface =
        const_cast<Corkscrew *>(this)->getOrCreateInputSurface();
    target_surface->clear();
    ...
    for (fl::size led_idx = 0; led_idx < mNumLeds; ++led_idx) {
        ...
        for (fl::u8 x = 0; x < 2; x++) {
            for (fl::u8 y = 0; y < 2; y++) {
                const auto &entry = tile.at(x, y);
                vec2<u16> pos =
                    entry.first; // position is the first element of the pair
                fl::u8 weight =
                    entry.second; // weight is the second element of the pair
...
fl::ScreenMap Corkscrew::toScreenMap(float diameter) const {
    ...
    for (fl::u16 i = 0; i < mNumLeds; ++i) {
        // Get the wrapped 2D position for this LED index in the cylindrical
        // mapping
...
fl::Grid<CRGB> &Corkscrew::surface() { return *getOrCreateInputSurface(); }

fl::size Corkscrew::pixelCount() const {
    // Use variant storage if available, otherwise fall back to legacy buffer
    // size
    if (!mPixelStorage.empty()) {
        if (mPixelStorage.template is<fl::span<CRGB>>()) {
