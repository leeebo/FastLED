...
#define FL_DISABLE_WARNING(warning)                                            \
    _Pragma(FL_STRINGIFY(clang diagnostic ignored "-W" #warning))
...
// Usage: FL_DISABLE_WARNING(float-equal)
#define FL_DISABLE_WARNING(warning)                                            \
    _Pragma(FL_STRINGIFY(GCC diagnostic ignored "-W" #warning))
#else
...
#define FL_DISABLE_WARNING_SELF_ASSIGN FL_DISABLE_WARNING(self-assign)
...
#define FL_DISABLE_FORMAT_TRUNCATION FL_DISABLE_WARNING(format-truncation)
#define FL_DISABLE_WARNING_NULL_DEREFERENCE
#define FL_DISABLE_WARNING_UNUSED_PARAMETER                                    \
    FL_DISABLE_WARNING(unused-parameter)
#define FL_DISABLE_WARNING_RETURN_TYPE FL_DISABLE_WARNING(return-type)

// implicit-fallthrough warning requires GCC >= 7.0
#if FL_GCC_VERSION >= 700
#define FL_DISABLE_WARNING_IMPLICIT_FALLTHROUGH                                \
    FL_DISABLE_WARNING(implicit-fallthrough)
#else
#define FL_DISABLE_WARNING_IMPLICIT_FALLTHROUGH
#endif
...
#define FL_FAST_MATH_BEGIN                                                     \
    _Pragma("clang diagnostic push") _Pragma("STDC FP_CONTRACT ON")

#define FL_FAST_MATH_END _Pragma("clang diagnostic pop")

#elif defined(FL_IS_GCC)
#define FL_FAST_MATH_BEGIN                                                     \
    _Pragma("GCC push_options") _Pragma("GCC optimize (\"fast-math\")")        \
...
#define FL_OPTIMIZATION_LEVEL_O3_BEGIN                                         \
    _Pragma("clang diagnostic push") _Pragma("clang optimize on")

#define FL_OPTIMIZATION_LEVEL_O3_END                                           \
    _Pragma("clang optimize off") _Pragma("clang diagnostic pop")
...
#if defined(FL_IS_CLANG)
#define FL_OPTIMIZATION_LEVEL_O0_BEGIN _Pragma("clang diagnostic push")
...
// Default level is O2 for balance between performance and timing
// predictability. Override with: #define FL_TIMING_OPT_LEVEL 3 before including
// this header.
...
// to build the optimization level string. Instead, we use conditional
// compilation.
...
#define FL_BEGIN_OPTIMIZE_FOR_EXACT_TIMING                                     \
    _Pragma("GCC push_options")                                                \
        _FL_TIMING_OPT_PRAGMA _Pragma("GCC optimize(\"-fno-lto\")")            \
...
#define FL_END_OPTIMIZE_FOR_EXACT_TIMING _Pragma("GCC pop_options")
#else
...
// Clang doesn't support optimize("O3"), but supports hot attribute for
...
// this pragma and will emit warnings. The macro expands to nothing on
...
//
// The first parameter provides a unique name for the generated wrapper
// function, ensuring no symbol collisions across the codebase.
//
...
//   - Sections are ordered lexicographically: .CRT$XCA, .CRT$XCU (user code),
//   .CRT$XCZ
//   - The function pointer must have static linkage to avoid ODR issues
//   - Works in static libraries if object file is linked (use /WHOLEARCHIVE if
//   needed)
...
#define FL_INIT(wrapper_name, func)                                            \
    ...
    __declspec(allocate(".CRT$XCU")) static void (*wrapper_name##_ptr)(void) = \
        wrapper_name;                                                          \
    }
...
#define FL_INIT(wrapper_name, func)                                            \
    namespace static_init {                                                    \
    FL_CONSTRUCTOR FL_KEEP_ALIVE void wrapper_name() { func(); }               \
    }
...
// FL_RUN_ONCE - Ensures a function is called only once (thread-safe on
// platforms with mutexes)
//
// Usage:
//   void initialize_system() { /* code */ }
//   FL_RUN_ONCE(initialize_system());
//
// This macro uses a static boolean flag to ensure the wrapped code executes
// only once, even if called multiple times. The implementation is thread-safe
// on platforms that support multithreading (via FASTLED_MULTITHREADED).
//
// Note: This is a runtime guard (checked each call), unlike FL_INIT which runs
// at static initialization time. FL_RUN_ONCE is useful for lazy initialization
// or when the caller doesn't know if initialization has occurred.
...
//
// Platform behavior:
//   ESP32: Places code in internal SRAM with unique section names
//   (.iram1.text.N) ESP8266: Places code in internal SRAM with unique section
//   names (.iram.text.N) STM32: Places code in fast RAM with unique section
//   names (.text_ram.N) Other platforms: No-op (functions execute from normal
//   memory)
//
...
// C++11 constexpr functions cannot have local variables or multiple return
// statements. These restrictions were relaxed in C++14. For C++11
// compatibility, we use inline functions instead of constexpr for complex
// functions.
...
// Mark functions whose return value must be used (generates compiler warning if
// ignored) Preferred over [[nodiscard]] for C++11/14 compatibility
//
...
//
// Note: __PRETTY_FUNCTION__ (GCC/Clang) includes full signature but is too
// verbose
//
// FL_PRETTY_FUNCTION: Full function signature including template parameters.
// Use when you need a unique string per template instantiation (e.g., singleton
// keys).
...
// Both arguments must be string literals or macros that expand to string
// literals.
...
// Detect if compiling with AddressSanitizer (ASAN includes LSAN -
// LeakSanitizer) GCC: __SANITIZE_ADDRESS__ is defined when -fsanitize=address
// is used Clang: __has_feature(address_sanitizer) evaluates to 1
