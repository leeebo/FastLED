...
/// Naive fallback solution for low level pin access using Arduino PINMAP
/// functions
class Pin : public Selectable {
    ...
    void _init() {
#if defined(digitalPinToBitMask) && defined(portOutputRegister) &&             \
    defined(portInputRegister)
...
#else
        // Fallback for platforms without PINMAP functions (e.g., STM32 Mbed
        // Arduino)
        mPinMask = 1;
        mPort = nullptr;
        mInPort = nullptr;
  ...
  public:
...
#ifndef __AVR__
    virtual ~Pin() {
    } // Shut up the compiler warning, but don't steal bytes from AVR.
#endif

    typedef volatile RwReg
        *port_ptr_t; ///< type for a pin read/write register, volatile
    typedef fl::u32
        port_t; ///< type for a pin read/write register, non-volatile
    ...
    inline void hi() __attribute__((always_inline)) {
        if (mPort) {
            *mPort |= mPinMask;
            ...
            digitalWrite(mPin, PinValue::High);
        }
    }
    ...
    inline void lo() __attribute__((always_inline)) {
        if (mPort) {
            *mPort &= ~mPinMask;
            ...
            digitalWrite(mPin, PinValue::Low);
        }
    }
    ...
    inline void toggle() __attribute__((always_inline)) {
        if (mInPort) {
            *mInPort = mPinMask;
            ...
            digitalWrite(mPin, current == PinValue::High ? PinValue::Low
                                                         : PinValue::High);
        }
    }
    ...
    FL_DISABLE_WARNING_PUSH
    FL_DISABLE_WARNING_VOLATILE
    inline void hi(FASTLED_REGISTER port_ptr_t port)
        __attribute__((always_inline)) {
        *port |= mPinMask;
    }
    /// Set the same pin on another port to `LOW`
    /// @param port the port to modify
    inline void lo(FASTLED_REGISTER port_ptr_t port)
        __attribute__((always_inline)) {
        *port &= ~mPinMask;
    }
    FL_DISABLE_WARNING_POP
    ...
    /// @note This function is not limited to the current pin! It modifies the
    /// entire register.
    FL_DISABLE_WARNING_PUSH
    FL_DISABLE_WARNING_VOLATILE
    inline void set(FASTLED_REGISTER port_t val)
        __attribute__((always_inline)) {
        *mPort = val;
    }
    ...
    inline void fastset(FASTLED_REGISTER port_ptr_t port,
    ...
    virtual bool isSelected() override {
        if (mPort) {
            ...
            return digitalRead(mPin) == PinValue::High;
        }
    }
};
