...
template <u8 PIN, u32 MASK, bool VALIDPIN> class _ESPPIN {
  public:
    ...
    static constexpr u32 GPIO_BIT_SET_REG =
        PIN < 32 ? GPIO_OUT_W1TS_REG : GPIO_OUT1_W1TS_REG;
    static constexpr u32 GPIO_BIT_CLEAR_REG =
        PIN < 32 ? GPIO_OUT_W1TC_REG : GPIO_OUT1_W1TC_REG;
#endif

    inline static void setOutput() {
        static_assert(validpin(),
                      "This pin has been marked as an invalid pin, common "
                      "reasons includes it being a ground pin, read only, or "
                      "too noisy (e.g. hooked up to the uart).");
        pinMode(PIN, PinMode::Output);
    }
    inline static void setInput() { pinMode(PIN, PinMode::Input); }

    inline static void hi() __attribute__((always_inline)) { *sport() = MASK; }

    inline static void lo() __attribute__((always_inline)) { *cport() = MASK; }
...
#define FASTLED_UNUSABLE_PIN_MASK                                              \
    (0ULL | _FL_BIT(6) | _FL_BIT(7) | _FL_BIT(8) | _FL_BIT(9) | _FL_BIT(10) |  \
     _FL_BIT(11) | _FL_BIT(20))
...
#define FASTLED_UNUSABLE_PIN_MASK                                              \
    (0ULL | _FL_BIT(11) | _FL_BIT(12) | _FL_BIT(13) | _FL_BIT(14) |            \
     _FL_BIT(15) | _FL_BIT(16) | _FL_BIT(17))
...
// NOTE: GPIO 37 & 38 commonly used for UART and may cause flashes when
// uploading.
#define FASTLED_UNUSABLE_PIN_MASK                                              \
    (0ULL | _FL_BIT(27) | _FL_BIT(28) | _FL_BIT(29) | _FL_BIT(30) |            \
     _FL_BIT(31) | _FL_BIT(32))
...
// GPIO 27-32 used by default for SPI flash.
// NOTE: GPIO 43 & 44 commonly used for UART and may cause flashes when
// uploading.
#define FASTLED_UNUSABLE_PIN_MASK                                              \
    (0ULL | _FL_BIT(19) | _FL_BIT(20) | _FL_BIT(27) | _FL_BIT(28) |            \
     _FL_BIT(29) | _FL_BIT(30) | _FL_BIT(31) | _FL_BIT(32))

#elif defined(FL_IS_ESP_32C5)

// GPIO 13,14 (USB_D*), 15-18,20-22 used by default for SPI flash.
#define FASTLED_UNUSABLE_PIN_MASK                                              \
    (0ULL | _FL_BIT(13) | _FL_BIT(14) | _FL_BIT(15) | _FL_BIT(16) |            \
     _FL_BIT(17) | _FL_BIT(18) | _FL_BIT(20) | _FL_BIT(21) | _FL_BIT(22))

#elif defined(FL_IS_ESP_32C6)

// GPIO 20-22, 24-26 used by default for SPI flash.
#define FASTLED_UNUSABLE_PIN_MASK                                              \
    (0ULL | _FL_BIT(24) | _FL_BIT(25) | _FL_BIT(26) | _FL_BIT(28) |            \
     _FL_BIT(29) | _FL_BIT(30))

#elif defined(FL_IS_ESP_32P4)
// 55 GPIO pins. ESPIDF defines all pins as valid.
// NOTE: GPIO 24 & 25 commonly used for USB and may cause flashes when
// uploading.
...
#define FASTLED_UNUSABLE_PIN_MASK                                              \
    (0ULL | _FL_BIT(15) | _FL_BIT(16) | _FL_BIT(17) | _FL_BIT(18) |            \
     _FL_BIT(19) | _FL_BIT(20) | _FL_BIT(21))
#elif defined(FL_IS_ESP_32C2)
// 21 GPIO pins (GPIO 0-20). ESPIDF defines all pins as valid.
// GPIO 11-17 are typically reserved for SPI flash and not recommended for other
// uses. NOTE: ESP32-C2 is a low-memory device - complex examples may not
// compile
#define FASTLED_UNUSABLE_PIN_MASK                                              \
    (0ULL | _FL_BIT(11) | _FL_BIT(12) | _FL_BIT(13) | _FL_BIT(14) |            \
     _FL_BIT(15) | _FL_BIT(16) | _FL_BIT(17))
...
// SOC GPIO mask was not added until version IDF version 4.3.  Prior to this
// only ESP32 chip was supported, so only the value for ESP32
#if ESP_IDF_VERSION < ESP_IDF_VERSION_VAL(4, 3, 0) &&                          \
    !defined(SOC_GPIO_VALID_OUTPUT_GPIO_MASK)
// 0~39 except from 24, 28~31 are valid
#define SOC_GPIO_VALID_GPIO_MASK                                               \
    (0xFFFFFFFFFFULL & ~(0ULL | _FL_BIT(24) | _FL_BIT(28) | _FL_BIT(29) |      \
                         _FL_BIT(30) | _FL_BIT(31)))
// GPIO >= 34 are input only
#define SOC_GPIO_VALID_OUTPUT_GPIO_MASK                                        \
    (SOC_GPIO_VALID_GPIO_MASK &                                                \
     ~(0ULL | _FL_BIT(34) | _FL_BIT(35) | _FL_BIT(36) | _FL_BIT(37) |          \
       _FL_BIT(38) | _FL_BIT(39)))

#endif

// Define mask of valid pins.  Start with the list of valid output pins from
// ESPIDF and remove unusable pins
#define _FL_VALID_PIN_MASK                                                     \
    (u64(SOC_GPIO_VALID_OUTPUT_GPIO_MASK) & ~FASTLED_UNUSABLE_PIN_MASK)

#define _FL_PIN_VALID(PIN) ((_FL_VALID_PIN_MASK & (1ULL << PIN)) != 0)

#define _FL_DEFPIN(PIN)                                                        \
    template <>                                                                \
    class FastPin<PIN>                                                         \
        : public _ESPPIN<PIN, ((u32)1 << (PIN % 32)), _FL_PIN_VALID(PIN)> {};

// Define all possible pins.  If the pin is not valid for a particular ESP32
// variant, the pin number will be shifted into the 192-255 range, in effect
