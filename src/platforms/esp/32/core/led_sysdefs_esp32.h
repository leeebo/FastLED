...
// Define missing version macros.  Hard code older version 3.0 since actual
// version is unknown
...
// Platform-specific IRAM attribute for ISR handlers and interrupt-sensitive
// functions ESP32: Places code in internal SRAM for fast, interrupt-safe
// execution Uses __COUNTER__ to generate unique section names (.iram1.text.0,
// .iram1.text.1, etc.) for better debugging and linker control
#ifndef FL_IRAM
...
// Generate unique section name using __COUNTER__ (e.g., .iram1.text.0,
// .iram1.text.1)
#define _FL_IRAM_SECTION_NAME(counter)                                         \
    ".iram1.text." _FL_IRAM_STRINGIFY(counter)
