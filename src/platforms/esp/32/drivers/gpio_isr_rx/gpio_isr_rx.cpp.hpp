...
inline bool isValidGpioPin(int pin) {
    ...
    // Check against the valid pin mask (excludes unusable pins like UART,
    // flash, etc.)
 ...
 */
inline int decodePulseBit(u32 high_ns, u32 low_ns,
 ...
 * @param edges Span of captured edge timestamps (spurious edges already
 * filtered upstream)
 * @param bytes_out Output span for decoded bytes
 *
 * Note: Edge detection/filtering must happen upstream when edges are stored.
 * This function assumes continuous LOW samples at the beginning have been
 * skipped.
 */
fl::Result<u32, DecodeError>
decodeEdgeTimestamps(const ChipsetTiming4Phase &timing,
    ...
    FL_LOG_RX("decodeEdgeTimestamps: decoding "
              << edge_count << " edges into buffer of " << bytes_capacity
              << " bytes");

    // Log first 100 edges for detailed analysis (using FL_WARN to ensure
    // output)
    size_t log_edge_limit = (edge_count < 100) ? edge_count : 100;
    for (size_t idx = 0; idx < log_edge_limit; idx++) {
        FL_WARN("Edge[" << idx << "]: time_ns=" << edges[idx].time_ns
                        << " level=" << static_cast<int>(edges[idx].level));
    }
    ...
    while (i + 2 < edge_count) {
        ...
        if (edge0.level == 0 && pulse0_duration >= reset_min_ns) {
            FL_LOG_RX("decodeEdgeTimestamps: reset pulse detected at edge "
                      << i);

            // Flush partial byte if needed
            if (bit_index != 0) {
                FL_WARN(
                    "decodeEdgeTimestamps: partial byte at reset (bit_index="
                    << bit_index << ")");
        ...
        // Gap tolerance: LOW duration extended beyond normal but within
        // tolerance Decode bit using only HIGH duration, ignore extended LOW
        else if (timing.gap_tolerance_ns > 0 && low_ns > timing.t0l_max_ns &&
                 low_ns <= timing.gap_tolerance_ns) {
            // Try to decode bit from HIGH duration only (LOW is extended by
            // gap)
            if (high_ns >= timing.t0h_min_ns && high_ns <= timing.t0h_max_ns) {
                bit = 0;
                FL_LOG_RX("decodeEdgeTimestamps: gap detected at edge "
                          << i << " (low=" << low_ns
                          << "ns), decoded bit 0 from high duration");
            } else if (high_ns >= timing.t1h_min_ns &&
                       high_ns <= timing.t1h_max_ns) {
                bit = 1;
                FL_LOG_RX("decodeEdgeTimestamps: gap detected at edge "
                          << i << " (low=" << low_ns
                          << "ns), decoded bit 1 from high duration");
            }
        }

        if (bit < 0) {
            error_count++;
            FL_LOG_RX("decodeEdgeTimestamps: invalid pulse at edge "
                      << i << " (high=" << high_ns << "ns, low=" << low_ns
                      << "ns)");
            i += 2;
            continue;
        }
        ...
        // Log detailed info for first 3 bytes (24 bits = first LED's RGB) -
        // using FL_WARN to ensure output
        if (bytes_decoded < 3) {
            FL_WARN("Bit[byte=" << bytes_decoded << ", bit=" << (bit_index - 1)
                                << "]: value=" << bit << " (high=" << high_ns
                                << "ns, low=" << low_ns << "ns) current_byte=0x"
                                << fl::hex << static_cast<int>(current_byte)
                                << fl::dec);
        }
        ...
        if (bit_index == 8) {
            if (bytes_decoded >= bytes_capacity) {
                FL_WARN("decodeEdgeTimestamps: buffer overflow at byte "
                        << bytes_decoded);
                return fl::Result<u32, DecodeError>::failure(
                    DecodeError::BUFFER_OVERFLOW);
            }
            ...
            // Log completed byte for first 3 bytes - using FL_WARN to ensure
            // output
            if (bytes_decoded <= 3) {
                FL_WARN("Byte[" << (bytes_decoded - 1) << "] completed: 0x"
                                << fl::hex << static_cast<int>(current_byte)
                                << fl::dec);
            }
...
struct FL_ALIGNAS(64) IsrContext {
    ...
    EdgeTimestamp
        *endPtr;       ///< End of buffer (read every edge for overflow check)
    u32 gpioInRegAddr; ///< GPIO_IN_REG or GPIO_IN1_REG address (read every
                       ///< call)
...
class GpioIsrRxImpl : public GpioIsrRx {
  public:
    GpioIsrRxImpl(int pin)
        : mIsrCtx{}, mPin(static_cast<gpio_num_t>(pin)), mBufferSize(0),
          mEdgeBuffer(), mIsrInstalled(false), mNeedsConversion(false),
          mSignalRangeMinNs(100), mSignalRangeMaxNs(100000), mStartLow(true) {
        FL_LOG_RX("GpioIsrRx constructed with pin="
        ...
        mIsrCtx.timeoutCycles =
            24000; // Default ~100us @ 240MHz, updated in begin()
        mIsrCtx.minPulseCycles =
            24; // Default ~100ns @ 240MHz, updated in begin()
    ...
    bool begin(const RxConfig &config) override {
        ...
        if (mIsrCtx.hwTimer == nullptr) {
            ...
            if (config.buffer_size == 0) {
                FL_WARN("GPIO ISR RX begin: Invalid buffer_size in config "
                        "(buffer_size=0)");
                return false;
            }
            ...
            if (!isValidGpioPin(static_cast<int>(mPin))) {
                FL_ERROR("GPIO ISR RX: Invalid pin "
                         << static_cast<int>(mPin)
                         << " - pin is reserved for UART, flash, or other "
                            "system use. "
            ...
            FL_LOG_RX("GPIO ISR RX first-time init: pin="
                      << static_cast<int>(mPin)
        ...
        if (mIsrCtx.minPulseCycles == 0)
            mIsrCtx.minPulseCycles = 1; // Minimum 1 cycle
        mIsrCtx.skipCounter = config.skip_signals;

        FL_LOG_RX("GPIO ISR RX begin: signal_range_min="
                  << mSignalRangeMinNs
        ...
        if (mIsrCtx.hwTimer == nullptr) {
            ...
            timer_config.resolution_hz = 1000000; // 1MHz = 1µs resolution

            esp_err_t err = gptimer_new_timer(&timer_config, &mIsrCtx.hwTimer);
            if (err != ESP_OK) {
                FL_WARN("Failed to create hardware timer: "
                        << static_cast<int>(err));
                return false;
            }
            ...
            cbs.on_alarm = timerPollingISR;
            err = gptimer_register_event_callbacks(mIsrCtx.hwTimer, &cbs,
                                                   &mIsrCtx);
            if (err != ESP_OK) {
                FL_WARN("Failed to register timer callback: "
                        << static_cast<int>(err));
                gptimer_del_timer(mIsrCtx.hwTimer);
                mIsrCtx.hwTimer = nullptr;
                return false;
        ...
        if (mIsrInstalled) {
            ...
            // Check if ISR is still armed from previous capture (error
            // condition)
            if (!mIsrCtx.receiveDone) {
                FL_ERROR(
                    "Timer ISR is still armed from previous capture - call "
                    "wait() or check finished() before calling begin() again");
                return false;
            }
        ...
        // Verify ISR is armed by checking that receiveDone is false (initial
        // state)
        if (mIsrCtx.receiveDone) {
            FL_WARN("Timer ISR started but receiver state is invalid "
                    "(receiveDone=true)");
            return false;
        }

        FL_LOG_RX("Timer ISR started successfully - polling at 2µs intervals "
                  "for ±1µs precision");
        ...
        while (!finished()) {
            ...
            if (elapsed_us >= timeout_us) {
                FL_WARN("wait(): timeout after "
                        << elapsed_us << "us, captured " << mIsrCtx.edgesCounter
                        << " edges");
                return RxWaitResult::TIMEOUT;
            }
    ...
    fl::span<const EdgeTimestamp> getEdges() const override {
        ...
        if (mNeedsConversion) {
            ...
            EdgeTimestamp *buffer =
                const_cast<EdgeTimestamp *>(mEdgeBuffer.data());
        ...
        return fl::span<const EdgeTimestamp>(mEdgeBuffer.data(),
                                             mIsrCtx.edgesCounter);
    }
  ...
  private:
     ...
     */
    FL_IRAM static bool __attribute__((optimize("O3"), hot))
    timerPollingISR(gptimer_handle_t timer,
        ...
        // Read pin state directly from register FIRST (fastest method - uses
        // precomputed address & mask)
        ...
        if (__builtin_expect(is_first_edge, 0)) {
            ...
            // Noise filter: reject pulses shorter than minimum (skip on first
            // edge)
            u32 cycles_since_last = now_cycles - ctx->lastEdgeCycles;
    ...
    size_t getRawEdgeTimes(fl::span<EdgeTime> out, size_t offset = 0) override {
        ...
        for (size_t i = 0; i < edges.size();) {
            ...
            size_t next_i = i + 1;
            while (next_i < edges.size() &&
                   edges[next_i].level == current_state) {
                next_i++;
            }
        ...
        if (start_edge >= total_edges)
            return 0;
        if (end_edge > total_edges)
            end_edge = total_edges;
        ...
        for (size_t i = 0; i < edges.size() && out_index < out.size();) {
            ...
            if (accumulated_ns >= mSignalRangeMinNs) {
                if (current_edge >= start_edge && current_edge < end_edge) {
                    out[out_index] =
                        EdgeTime(current_state == 1, accumulated_ns);
                    out_index++;
                }
                current_edge++;
    ...
    bool injectEdges(fl::span<const EdgeTime> edges) override {
        ...
        if (mEdgeBuffer.empty()) {
            ...
            FL_WARN("injectEdges(): edge buffer too small (need "
                    << edges.size() << ", have " << mEdgeBuffer.size() << ")");
            return false;
        }

        FL_LOG_RX("injectEdges(): injecting "
                  << edges.size() << " edges into GPIO ISR RX buffer");
        ...
        FL_LOG_RX("injectEdges(): injected " << edges.size()
                                             << " edges successfully");
        return true;
    }
    ...
    fl::vector<EdgeTimestamp>
        mEdgeBuffer; ///< Buffer (ISR writes cycles, getEdges() converts to ns)
    bool mIsrInstalled;    ///< True if ISR handler is installed
    bool mNeedsConversion; ///< True if buffer has cycles (not yet converted to
                           ///< ns)
